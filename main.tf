...
resource "local_file" "ansible_inventory" {
  ...
  content = <<-EOT
    ...
    [all:vars]
    domain_name=${var.stack_config.domain_name}
    ...
    container_id=${var.ct_config.id}
    container_ip=${cidrhost(var.ct_network.network, var.ct_config.hostnum)}
    hostname=${var.ct_config.hostname}
    keycloak_admin=admin
    keycloak_admin_pass=${var.keycloak_admin_pass}
    keycloak_postgresql_pass=dae19943-4166-4cc6-b338-728896c48656
    minio_admin_pass=${var.minio_admin_pass}
EOT
}
